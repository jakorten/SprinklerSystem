...
Note: this file can be used to use in stand alone mode (without e.g. the controlServer)
...
if __name__ == '__main__':
    try:
        pi = pigpio.pi()
        if not pi.connected:
            print("Start pigpio deamon first!")
            exit()
        print("======================================")
        print(" System reports: pigpio deamon found!")

        print(" 5x Motor Controller HAT started.")
        testControl = tapHatCinqController()
        if (testControl._error == False):
            print(" Communication with PCA9685 OK")
            print("======================================")
        else:
            sys.exit(0)
        motor = testControl.motor5
        while True:
            motor.motorForward()
            print("motorForward")
            time.sleep(5)
        ...
        testControl.stopMotors() # note: do not forget this otherwise motor might still be powered...
        try:
            sys.exit(0)
        except SystemExit:
            os._exit(0)
...
result = findPCA9685()
if (result):
    print("Found at address " + str(hex(deviceAddress)))
else:
    print("Not found")
'''
